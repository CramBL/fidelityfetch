...
help() {
    cat <<'EOF'
Install a binary release of a Fidelity Fetch (fife) hosted on GitHub
...
CRATE=fidelityfetch
URL=https://github.com/CramBL/fidelityfetch
...
if [ -z "${tag-}" ]; then
    tag=$(
        download https://api.github.com/repos/Crambl/fidelityfetch/releases/latest - |
            grep tag_name |
            cut -d'"' -f4
    )
fi
